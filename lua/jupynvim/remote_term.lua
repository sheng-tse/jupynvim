...
local function ensure_inputrc(client)
  if client._jupynvim_inputrc ~= nil then
    return client._jupynvim_inputrc or nil
  end
  local script = 'd="$HOME/.local/share/jupynvim" && mkdir -p "$d" && cat > "$d/inputrc" <<\'EOF\'\n'
    .. INPUTRC_BODY .. '\nEOF\nprintf \'%s\' "$d/inputrc"'
  ...
  client._jupynvim_inputrc = path or false
  return path
end
